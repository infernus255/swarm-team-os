# Plan de Implementaci├│n: Instalaci├│n de Hermes Agent y Conexi├│n con Telegram en Ubuntu (Codespaces)

Este documento contiene instrucciones paso a paso para instalar Hermes Agent, configurar un Gateway de Telegram y asegurar la replicabilidad en m├║ltiples entornos.

---

## 1. Objetivo

Construir una instalaci├│n completa y reproducible de Hermes Agent en un entorno Ubuntu/Codespaces, con:
...
- Diagn├│stico y reparaci├│n de errores de proveedor
...
- `TELEGRAM_BOT_TOKEN` ÔÇö generado por `@BotFather` con `/newbot`
- `TELEGRAM_USER_ID` ÔÇö tu ID de Telegram obtenido con `@userinfobot`
- `GEMINI_API_KEY` or `GOOGLE_API_KEY` ÔÇö clave de Google Gemini / AI Studio
- Acceso a la terminal de Ubuntu / Codespaces
- `curl`, `git`, `python3`, `npm` disponibles

> Mant├®n las claves fuera de chat y en un archivo `.env` local. Nunca compartas claves reales en mensajes.
...
Verifica que `python3`, `pip`, `npm` y `curl` est├®n instalados:
...
Despu├®s de la instalaci├│n, recarga tu shell para asegurar que `~/.local/bin` est├® en el `PATH`:
...
## 5. Configuraci├│n segura de credenciales
...
> `GATEWAY_ALLOW_ALL_USERS=true` debe usarse s├│lo durante pruebas. En producci├│n, define `TELEGRAM_ALLOWED_USERS`.

---

## 6. Configuraci├│n de Hermes para Google Gemini directo
...
Verifica el estado de la configuraci├│n:
...
## 7. Configuraci├│n del Gateway de Telegram
...
5. Para ejecuci├│n en Codespaces, evitar systemd si no est├í disponible

Verifica el Gateway y config├║ralo:
...
### Opci├│n A: PM2 (recomendada dentro de Codespaces)
...
Guarda la configuraci├│n de PM2:
...
### Opci├│n B: Servicio systemd (si el host lo soporta)
...
### Opci├│n C: Docker / contenedor reproducible
...
## 9. Diagn├│stico y soluci├│n de errores comunes
...
### 9.2 Verificar la configuraci├│n actual
...
Si Hermes muestra advertencias de versi├│n de config, ejecuta:
...
3. Si hay un nuevo aprendizaje, agr├®galo a la memoria:
...
Este autorunner est├í dise├▒ado para ser seguido literalmente por una IA de bajo tier.
<!-- AUTORUNNER-END -->

### 10.1 Uso de scripts de instalaci├│n reproducibles

Crea un script `hermes-install.sh` que incluya:

- instalaci├│n de dependencias
- descarga de Hermes
- creaci├│n de `~/.hermes/.env`
- configuraci├│n de `model.provider`, `model.default`, `model.base_url`
- arranque del gateway

### 10.2 Uso de Docker y `.env` para m├║ltiples entornos

Mant├®n una plantilla de `.env` borrada en el repositorio y un `Dockerfile` que s├│lo copie el `.env` del entorno en tiempo de despliegue.

### 10.3 Extender funcionalidades

Una vez Hermes y Telegram funcionan, puedes a├▒adir:

- `TELEGRAM_ALLOWED_USERS` para control de acceso
- `hermes config set telegram.commands` para comandos personalizados
- `pm2` o `systemd` para que el bot se reinicie autom├íticamente
- un contenedor Docker que reproduzca la misma configuraci├│n en AWS, GCP, DigitalOcean o un runner local
...
- [ ] `hermes gateway status` muestra gateway en ejecuci├│n
- [ ] `hermes doctor` no reporta errores de provider
- [ ] Telegram responde a comandos `/new` y mensajes b├ísicos
...
Mant├®n siempre las claves de proveedor fuera del chat.
Usa archivos locales `.env` y permisos restringidos (`chmod 600 ~/.hermes/.env`).

---

## 13. C├│mo usar este plan con un agente IA
...
> Este plan est├í escrito para ser completo y minimizar la necesidad de interpretaci├│n. Cualquier modelo IA que siga los comandos debe poder implementar Hermes + Telegram en Ubuntu y dejarlo listo para escalar.
...
- La imagen Docker se construy├│ con ├®xito usando `docker compose build`.
- El contenedor puede ejecutar Hermes y el binario se comprueba con `hermes --version`.
- La configuraci├│n de proveedor Gemini se aplica al arrancar el contenedor.

Notas finales de despliegue:

- Para que Telegram se conecte correctamente, `TELEGRAM_BOT_TOKEN` debe ser un token v├ílido de BotFather.
- El uso de `GATEWAY_ALLOW_ALL_USERS=true` es ├║til en pruebas, pero en producci├│n conviene definir `TELEGRAM_ALLOWED_USERS`.
